...
This column serves as indices. One number must be put into each row, beginning from 1 and incrementing by 1. Indices will be recreated in `loadConstraints()` if necessary.
...
* `Include` specifies the items matching the condition to be included in selection. For example, the following row tells the solver to include items `SC00003` and `SC00004`:
...
* `Exclude` specifies the items matching the condition to be excluded in selection. The following row tells the solver to exclude items that match `PTBIS < 0.15`, based on the supplied item attributes.
