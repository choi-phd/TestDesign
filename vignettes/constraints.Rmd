...
This document explains how to create constraints for `loadConstraints()`. In test assembly, practitioners often want to select the items that satify various types of constraints. As of *TestDesign* version 1.1.0, constraints can be read in from a data.frame object or a `.csv` spreadsheet file. Constraints are expected to be in the following structure:
...
Constraints should be structured in seven columns, named as `CONSTRAINT`, `TYPE`, `WHAT`, `CONDITION`, `LB`, `UB`, `ONOFF` on the first row. Beginning from the second row, each row should have the corresponding values for the 7 columns. A convenient way to creating constraints is to use a spreadsheet application (e.g. Excel), work on the contents from there, and then saving it as a `.csv` file.
...
This column serves as indices for the constraints. A single numeric value should be put into each row, incrementing by 1. Indices will be recreated in `loadConstraints()` if necessary.
