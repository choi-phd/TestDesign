---
title: "Creating constraints file in oat package"
author: Sangdon Lim
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Creating constraints file in oat package}
...
```{}
CONSTRAINT,TYPE,WHAT,CONDITION,LB,UB
1,Number,Item,,30,30
...
The constraint file should have 6 columns, named as `CONSTRAINT`, `TYPE`, `WHAT`, `CONDITION`, `LB`, `UB` on the first row of the file. Optionally, an extra column `ONOFF` can be created. This column can be used to toggle each constraint row for test development purposes.
...
This column can be used to turn off a constraint from being applied. The following row specifies the order condition to be not applied.
