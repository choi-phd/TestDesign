...
## Tell R where the Fortran compiler is located

The cause of the error is that R needs Fortran compiler to build the package, but it does not know where it is.

**Step 1.** Run the following lines in terminal.

```
mkdir ~/.R
cat << EOF >> ~/.R/Makevars
FLIBS = -L/usr/local/gfortran -lgfortran -lquadmath -lm
EOF
```
