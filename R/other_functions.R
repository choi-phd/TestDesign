...
#' @noRd
plotAuditTrail <- function(o, j) {

  constants <- TestDesign:::getConstants(
    o@constraints, o@config,
    o@data, o@true_theta
  )
  ...
  plot(1:max_ni, seq(min_q, max_q, length = max_ni),
    ...
    type = "n", las = 1)
  points(1:max_ni, o@output[[j]]@interim_theta_est,
    type = "b", pch = 9, col = "blue")

  if (!is.null(o@output[[j]]@true_theta)) {
    abline(h = o@output[[j]]@true_theta, lty = 2, col = "red")
  } else {
    abline(h = o@output[[j]]@final_theta_est, lty = 2, col = "red")
  }

  item_string <- paste(o@output[[j]]@administered_item_index, collapse = ",")
  text(1, max_q,
    paste0("Items: ", item_string),
    cex = 0.7, adj = 0)
  text(1, min_q + 0.3,
    paste("Theta: ", round(o@output[[j]]@final_theta_est, digits = 2), " SE: ", round(o@output[[j]]@final_se_est, digits = 2)),
    cex = 0.8, adj = 0)

  for (i in 1:max_ni) {
    lines(rep(i, 2), c(
      o@output[[j]]@interim_theta_est[i] - 1.96 * o@output[[j]]@interim_se_est[i],
      o@output[[j]]@interim_theta_est[i] + 1.96 * o@output[[j]]@interim_se_est[i]))
    if (constants$use_shadow) {
      if (o@output[[j]]@shadow_test_refreshed[i]) {
        points(i, o@output[[j]]@interim_theta_est[i], pch = 18, col = "red")
      }
    }
  }

  resp_string <- paste(o@output[[j]]@administered_item_resp, collapse = ",")
  plot(
    theta_grid, o@output[[j]]@posterior,
    main = "Final Posterior Distribution", xlab = "Theta", ylab = "Posterior",
    type = "l", col = "blue", yaxt = "n")
  text(
    min_q, max(o@output[[j]]@posterior),
    paste0("Responses: ", resp_string), cex = 0.7, adj = 0
  )
